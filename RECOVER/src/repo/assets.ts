...
export async function listAssets() {
  ...
  const rows: any[] = [];
  db.exec({ sql: 'SELECT * FROM assets ORDER BY created_at DESC', rowMode: 'object', callback: (r:any)=>rows.push(r) });
  return rows;
}
...
export async function listAssetsByOwner(ownerType: string, ownerId: string) {
  const db = await getDb();
  const rows: any[] = [];
  db.exec({ sql: 'SELECT * FROM assets WHERE owner_type=? AND owner_id=? ORDER BY created_at DESC', bind: [ownerType, ownerId], rowMode: 'object', callback: (r:any)=>rows.push(r) });
  return rows;
}
...
export async function updateAsset(id: string, patch: Partial<{ kind: string; path: string; meta_json: string }>) {
  ...
  db.exec({ sql: `UPDATE assets SET kind=?, path=?, meta_json=? WHERE id=?`, bind: [kind, path, meta_json, id] });
}

export async function deleteAsset(id: string) {
  const db = await getDb();
  db.exec({ sql: `DELETE FROM assets WHERE id=?`, bind: [id] });
}

export async function getAssetById(id: string) {
  const db = await getDb();
  const rows: any[] = [];
  db.exec({ sql: 'SELECT * FROM assets WHERE id=? LIMIT 1', bind: [id], rowMode: 'object', callback: (r:any)=>rows.push(r) });
  return rows[0] || null;
}
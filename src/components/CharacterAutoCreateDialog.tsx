...
const AUTO_CREATOR_VERSION = "2025.10.07";
...
const ARCHETYPE_OPTIONS: Array<{
...
}> = [
  {
    ...
    description:
      "Campus muse with upperclass swagger, juggling seminars, secret societies, and late-night DJ residencies. She weaponizes charisma, curated chaos, and a fully grown sense of freedom.",
  },
  {
    id: "prime",
    label: "Prime",
    ...
    description:
      "Powerhouse in her prime with a polished edge and a high-end grind. Think luxury marketing gigs, velvet ropes, and a magnetic presence that pulls every eye in the room.",
  },
  {
    id: "fresh",
    label: "Fresh",
    ...
    description:
      "Fresh-faced whirlwind with restless ambition and a hunger for spotlight moments. From viral dance crews to underground fashion drops, she brings adult mischief with a wide-eyed rush.",
  },
];
...
const GENDER_OPTIONS: Array<{
...
}> = [
  {
    ...
    label: "Girls",
    blurb:
      "Sultry muses who blend seduction with playfulness, fully aware of the power they hold and eager to wield it with intent.",
  },
  {
    id: "male",
    label: "Guys",
    blurb:
      "Smooth operators who treat the house like a five-star playground—confident, indulgent, and ready to spoil their match.",
  },
];
...
interface CharacterAutoCreateContentProps
  extends CharacterAutoCreateCommonProps {
  active: boolean;
  onClose: () => void;
  layout: "modal" | "inline";
}
...
function CharacterAutoCreateContent({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (saved) {
        ...
        toast.error(
          "A girl with that name already exists. Try adjusting the generation notes."
        );
      }
    } catch (error) {
      const message =
  ...
  const selectedArchetype = ARCHETYPE_OPTIONS.find(
    (option) => option.id === archetype
  );
    ...
    >
      <header className="flex flex-shrink-0 items-center justify-between border-b border-white/5 px-5 py-4">
        <div>
          <div className="flex items-center gap-2 text-[10px] uppercase tracking-[0.35em] text-white/40">
            ...
            <Badge
              variant="outline"
              className="rounded-full border-white/20 bg-white/5 px-2 py-0 text-[9px] font-semibold tracking-[0.2em] text-white/70"
            >
              v{AUTO_CREATOR_VERSION}
            </Badge>
          </div>
          <h2 className="mt-1 text-xl font-semibold text-white">
            Create a Tailored Character
          </h2>
          <p className="mt-1 text-xs text-white/55">
            Tune archetype, notes, and prompts while keeping the rest of the
            house visible.
          </p>
        </div>
      ...
      <div className="flex-1 min-h-0 overflow-hidden px-5 pb-5 pt-5">
        <div className="grid h-full min-h-0 gap-5 xl:grid-cols-[420px_minmax(0,1fr)]">
          <div
            className="space-y-6 overflow-y-auto rounded-2xl border border-white/10 bg-white/5 p-5 h-full"
            style={{
                ...
                {GENDER_OPTIONS.map((option) => {
                  ...
                  return (
                    <Tooltip key={option.id}>
                      ...
                      <TooltipContent
                        side="top"
                        className="max-w-[220px] text-left leading-relaxed text-white/85"
                      >
                        {option.blurb}
                      </TooltipContent>
                    </Tooltip>
          ...
          <div className="flex flex-col overflow-hidden rounded-2xl border border-white/10 bg-white/5 h-full min-h-0">
            {!draft ? (
              <div className="flex flex-1 items-center justify-center px-6 py-12 text-center text-sm text-white/55">
                Configure a gender and type, add any guidance, then generate to
                preview a fully-loaded profile.
              </div>
            ) : (
              <div
              ...
              >
                <div className="space-y-6 p-6 pb-24">
                  <header className="flex items-start justify-between gap-3">
                    ...
                    <Badge
                      variant="outline"
                      className="uppercase tracking-[0.22em] text-white/70"
                    >
                      {draft.rarity}
                    </Badge>
                  </header>
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="leading-relaxed text-sm text-white/70 whitespace-pre-line">
                      {draft.description}
                    </p>
                  </div>
                  ...
                  <div className="space-y-4 rounded-2xl border border-white/10 bg-[#080812] p-4">
                    ...
                    <PromptBlock
                      ...
                      value={draft.prompts?.description ?? ""}
                    />
                    <PromptBlock
                      title="Personality Prompt"
                      value={draft.prompts?.personality ?? ""}
                    />
                    <PromptBlock
                      title="Background Context"
                      value={draft.prompts?.background ?? ""}
                    />
                    <PromptBlock
                      title="Appearance Prompt"
                      value={draft.prompts?.appearance ?? ""}
                    />
                    <PromptBlock
                      title="Response Style"
                      value={draft.prompts?.responseStyle ?? ""}
                    />
                    <PromptBlock
                      title="Origin Scenario"
                      value={draft.prompts?.originScenario ?? ""}
                    />
                  </div>
                </div>
              </div>
...
interface CharacterAutoCreateDialogProps
  extends CharacterAutoCreateCommonProps {
  open: boolean;
...
interface CharacterAutoCreateInlineProps
  extends CharacterAutoCreateCommonProps {
  active: boolean;
  onClose: () => void;
}

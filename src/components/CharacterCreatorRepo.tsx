...
export function CharacterCreatorRepo({
  open,
  onOpenChange,
  character,
}: CharacterCreatorRepoProps) {
  const { addCharacter, updateCharacter, isLoading, characters } =
    useHouseFileStorage();
  ...
  const [availableAppearanceTags, setAvailableAppearanceTags] = useState<
    string[]
  >([]);
  ...
  const normalizeToOption = (
    ...
    const contains = options.find(
      (o) => syn.includes(o.toLowerCase()) || o.toLowerCase().includes(syn)
    );
    if (contains) return contains;
    // Title-case fallback if close
    const cap = s.replace(/\b\w/g, (c) => c.toUpperCase());
    const close = options.find(
      (o) =>
        o.toLowerCase().startsWith(s.toLowerCase()) ||
        s.toLowerCase().startsWith(o.toLowerCase())
    );
    return close || cap;
  };
  ...
  const mapCharacterToForm = (c: any) => {
    ...
    const appearanceTagsFromFeatures = Array.isArray(c.features)
      ? c.features
          .filter((f: any) => typeof f === "string" && !String(f).includes(":"))
          .map((s: string) => s.trim())
          .filter(Boolean)
      : [];
    ...
    return {
      name: c.name || "",
      gender:
        (c.gender as "female" | "male" | "other") ||
        (c.role === "guest" ? "male" : "female"),
      age: typeof c.age === "number" ? String(c.age) : "",
      job: c.job || "",
      ...
      traits: Object.keys(traitsFromJSON).length
        ? traitsFromJSON
        : c.traits || physicalTraits,
      tags:
        Array.isArray(tagsFromJSON) && tagsFromJSON.length
          ? tagsFromJSON
          : [...(c.tags || []), ...(c.personalities || [])].filter(Boolean),
  ...
  useEffect(() => {
    if (isEditing && character) {
      ...
      const hRaw =
        character?.physicalStats?.height || character?.traits?.height || "";
      const wRaw =
        character?.physicalStats?.weight || character?.traits?.weight || "";
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (draft.traits) {
        const t = { ...formData.traits, ...draft.traits } as Record<
          string,
          any
        >;
        ...
        if (t.hairColor)
          t.hairColor =
            normalizeToOption(t.hairColor, HAIR_COLOR_OPTIONS) || t.hairColor;
        if (t.eyeColor)
          t.eyeColor =
            normalizeToOption(t.eyeColor, EYE_COLOR_OPTIONS) || t.eyeColor;
        if (t.skinTone)
          t.skinTone =
            normalizeToOption(t.skinTone, SKIN_TONE_OPTIONS) || t.skinTone;
      ...
      toast.error(
        error instanceof Error
          ? error.message
          : "Failed to generate character details"
      );
    } finally {
  ...
  const handleSave = async () => {
    ...
    const nameExists = (characters || []).some(
      (c: Character) =>
        ...
        (!isEditing || c.id !== (character as Character)?.id)
    );
    if (nameExists) {
      toast.error(
    ...
    if (isMale) {
      ...
      if (
        formData.age !== "" &&
        (Number(formData.age) <= 0 || Number.isNaN(Number(formData.age)))
      ) {
      ...
      if (heightUnit === "cm") {
        finalHeightCm =
          typeof heightCm === "number"
            ? heightCm
            : parseHeightToCm(String(heightCm));
      } else {
        const ft =
          typeof heightFt === "number"
            ? heightFt
            : parseInt(String(heightFt || "0"), 10);
        const inch =
          typeof heightIn === "number"
            ? heightIn
            : parseInt(String(heightIn || "0"), 10);
      ...
      if (weightUnit === "kg") {
        finalWeightKg =
          typeof weightKg === "number"
            ? weightKg
            : parseWeightToKg(String(weightKg));
      } else {
        const lbsNum =
          typeof weightLbs === "number"
            ? weightLbs
            : parseFloat(String(weightLbs || ""));
        finalWeightKg = isNaN(lbsNum as number)
          ? null
          : Math.round((lbsNum as number) * 0.45359237);
      }
    }

    if (isEditing) {
      // Map repo-style fields back to in-app Character model for updates
      const physicalStatsFromTraits = isMale
        ? character?.physicalStats || undefined
        : {
            hairColor:
              formData.traits.hairColor ??
              character?.physicalStats?.hairColor ??
              "",
            eyeColor:
              formData.traits.eyeColor ??
              character?.physicalStats?.eyeColor ??
              "",
            height:
              finalHeightCm != null
                ? `${finalHeightCm} cm`
                : (character?.physicalStats?.height ?? ""),
            weight:
              finalWeightKg != null
                ? `${finalWeightKg} kg`
                : (character?.physicalStats?.weight ?? ""),
            skinTone:
              formData.traits.skinTone ??
              character?.physicalStats?.skinTone ??
              "",
          };

      const featuresFromTraits = isMale
        ? []
      ...
      const updates: Partial<Character> = {
        ...
        appearance:
          characterData.appearanceDescription || character?.appearance,
        imageDescription:
          characterData.appearanceDescription || character?.imageDescription,
        avatar: isMale ? character?.avatar : characterData.avatar_path,
        prompts: {
          ...currentPrompts,
          system: isMale
            ? ""
            : characterData.system_prompt || currentPrompts.system,
          description: characterData.bio || currentPrompts.description,
          background: characterData.bio || currentPrompts.background,
          appearance:
            characterData.appearanceDescription || currentPrompts.appearance,
        },
        ...
        features: isMale
          ...
          : Array.from(
              new Set([
                ...(character?.features || []),
                ...featuresFromTraits,
                ...formData.appearanceTags,
              ])
            ),
        personalities: Array.from(
          new Set([...(character?.personalities || []), ...formData.tags])
        ),
      ...
      const newCharacter: Character = {
        ...
        imageDescription:
          characterData.appearanceDescription || characterData.bio,
        physicalStats: isMale
          ? undefined
          : {
        ...
        prompts: {
          ...
          appearance: characterData.appearanceDescription,
          responseStyle: isMale
            ? ""
            : "Keep replies smooth, attentive, and aligned with the character’s adult fantasies.",
      ...
      const enrichedCharacter = await populateCharacterProfile(newCharacter, {
        request: `Use these repo-sourced facts to build consistent prompts for ${newCharacter.name}. Personality tags: ${formData.tags.join(", ")}. Appearance: ${newCharacter.appearance}. Bio: ${newCharacter.description}.`,
        name: newCharacter.name,
        existing: newCharacter,
        mode: "preserve",
      });

      const added = await addCharacter(enrichedCharacter);
      if (added) {
        try {
          ...
          logger.warn("Could not dynamically import useFileStorage:", e);
        }
        try {
          ...
          if (rows.length > 0) {
            try {
              images = JSON.parse(rows[0].value) || [];
            } catch {
              images = [];
            }
          }
          ...
          if (typeof avatarUrl === "string" && avatarUrl.trim().length > 0) {
            ...
            const before: any[] = [];
            db.exec({
              sql: "SELECT total_changes() AS c",
              rowMode: "object",
              callback: (r: any) => before.push(r),
            });
            db.exec({
          ...
          <div className="flex items-center justify-between w-full gap-4">
            <div>
              ...
              <div className="text-sm text-muted-foreground">
                Update profile, appearance, and AI prompts
              </div>
            </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div className="space-y-2">
                    ...
                    <Select
                      ...
                      onValueChange={(val: "female" | "male" | "other") =>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-3">
                      <Input
                        ...
                        onChange={(e) =>
                          updateFormData("avatar_path", e.target.value)
                        }
                      ...
                      <Button
                        ...
                        onClick={async () => {
                          ...
                          try {
                            ...
                            if (formData.traits.hairColor)
                              traitBits.push(
                                `${formData.traits.hairColor} hair`
                              );
                            if (formData.traits.eyeColor)
                              traitBits.push(
                                `${formData.traits.eyeColor} eyes`
                              );
                            if (formData.traits.skinTone)
                              traitBits.push(
                                `${formData.traits.skinTone} skin`
                              );
                            const traitDesc = traitBits.join(", ");
                            const appearance = [baseDesc, traitDesc]
                              .filter(Boolean)
                              .join(", ");
                            const selfieSuffix =
                              "selfie, portrait, profile picture, shoulders up, looking at camera, subtle smile, soft diffused lighting, clean blurred background, natural skin, high detail, 1:1 aspect, photographic";
                            const prompt = [
                              appearance || formData.bio || formData.name,
                              selfieSuffix,
                            ]
                              .filter(Boolean)
                              .join(", ");
                ...
                <div className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <Textarea
                      ...
                      onChange={(e) =>
                        updateFormData("appearanceDescription", e.target.value)
                      }
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex gap-2 items-center">
                      <Select
                        value={
                          formData.traits.hairColor
                            ? String(formData.traits.hairColor)
                            : undefined
                        }
                        onValueChange={(val) =>
                          setFormData((prev) => ({
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex gap-2 items-center">
                      <Select
                        value={
                          formData.traits.eyeColor
                            ? String(formData.traits.eyeColor)
                            : undefined
                        }
                        onValueChange={(val) =>
                          setFormData((prev) => ({
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex gap-2 items-center">
                      <Select
                        value={
                          formData.traits.skinTone
                            ? String(formData.traits.skinTone)
                            : undefined
                        }
                        onValueChange={(val) =>
                          setFormData((prev) => ({
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex flex-wrap gap-2 items-center">
                      <Select
                        value={weightUnit}
                        onValueChange={(v: "kg" | "lbs") => setWeightUnit(v)}
                      >
                        <SelectTrigger className="w-[140px]">
                          <SelectValue />
                        </SelectTrigger>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {Object.entries(formData.traits)
                        .filter(
                          ([key]) =>
                            !["hairColor", "eyeColor", "skinTone"].includes(key)
                        )
                        .map(([key, value]) => (
                          <Badge
                            key={key}
                  ...
                  <div className="space-y-2">
                    ...
                    {availableAppearanceTags.length > 0 && (
                      <div className="space-y-2">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {availableAppearanceTags
                            .filter((t) => !formData.appearanceTags.includes(t))
                            .slice(0, 24)
                            .map((t) => (
                              <Badge
                                key={t}
                                ...
                                onClick={() => {
                                  setFormData((prev) => ({
                                    ...prev,
                                    appearanceTags: Array.from(
                                      new Set([...prev.appearanceTags, t])
                                    ),
                                  }));
                                }}
                              >
                ...
                <div className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <Textarea
                      ...
                      onChange={(e) =>
                        updateFormData("guidance", e.target.value)
                      }
                  ...
                  <div className="space-y-4">
                    <div className="text-sm text-muted-foreground">
                      Use the single Generate button to enhance all character
                      fields based on the current information and guidance.
                    </div>
                    <Button
                      variant="outline"
                      className="w-full"

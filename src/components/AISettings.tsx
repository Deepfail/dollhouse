...
export function AISettings({ children }: AISettingsProps) {
  ...
  React.useEffect(() => {
    ...
    if (textProvider === "venice") {
      ...
      const nextModel = veniceValues.has(veniceTextModel)
        ? veniceTextModel
        : "venice-large";
      ...
      const veniceValues = new Set(veniceTextModels.map((m) => m.value));
      const nextModel = veniceValues.has(openrouterTextModel)
        ? "deepseek/deepseek-chat-v3.1"
        : openrouterTextModel;
  ...
  const loadSettings = async () => {
    try {
      ...
      const loadedVeniceKey =
        aiSettings.veniceTextApiKey ||
        (loadedProvider === "venice" ? aiSettings.textApiKey : "") ||
        "";
      const loadedOpenRouterKey =
        aiSettings.openrouterTextApiKey ||
        (loadedProvider === "openrouter" ? aiSettings.textApiKey : "") ||
        "";
      const loadedVeniceModel =
        aiSettings.veniceTextModel ||
        (aiSettings.textProvider === "venice"
          ? aiSettings.textModel
          : undefined) ||
        "venice-large";
      const loadedOpenRouterModel =
        aiSettings.openrouterTextModel ||
        (aiSettings.textProvider === "openrouter"
          ? aiSettings.textModel
          : undefined) ||
        "deepseek/deepseek-chat-v3.1";
      const loadedVeniceUrl =
        aiSettings.veniceTextApiUrl ||
        (aiSettings.textProvider === "venice"
          ? aiSettings.textApiUrl
          : undefined) ||
        "";
  ...
  const saveSettings = async () => {
    try {
      const houseConfig =
        ((await repositoryStorage.get("house_config")) as any) || {};
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent
        className="max-w-4xl max-h-[90vh] overflow-visible bg-gray-900 text-white"
                    ...
                    {imageProvider === "venice" && (
                      <div>
                        ...
                        >
                          ...
                          <SelectContent>
                            <SelectItem value="venice-sd35">
                              Venice SD3.5 - Default
                            </SelectItem>
                            <SelectItem value="hidream">
                              HiDream I1 - High quality
                            </SelectItem>
                            <SelectItem value="qwen-image">
                              Qwen Image - Highest quality
                            </SelectItem>
                            <SelectItem value="lustify-sdxl">
                              Lustify - Uncensored
                            </SelectItem>
                            <SelectItem value="wai-Illustrious">
                              Anime Uncensored
                            </SelectItem>
                            <SelectItem value="pony-realism">
                              Pony Uncensored
                            </SelectItem>
                            <SelectItem value="flux-dev">
                              High Quality
                            </SelectItem>
                            <SelectItem value="flux-dev-uncensored">

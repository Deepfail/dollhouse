...
export function WingmanSettings({ onClose }: { onClose?: () => void }) {
  ...
  const [activeTab, setActiveTab] = useState<"prompts" | "girls" | "interview">(
    "prompts"
  );
  ...
  return (
    <div className="p-4 bg-black/90 border border-white/10 rounded-xl w-[min(640px,90vw)] text-sm text-white space-y-4 max-h-[75vh] overflow-y-auto">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-white/60">
            View current prompts and character analysis.
          </p>
        </div>
        {onClose && (
      ...
      <div className="flex gap-2">
        ...
        <Button
          size="sm"
          variant={activeTab === "girls" ? "default" : "outline"}
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <p className="text-sm font-semibold text-blue-300">
                  Prompts Are Now Centrally Managed
                </p>
                <p className="text-xs text-blue-200/80 mt-1">
                  All Wingman prompts (system, greeting, interview) are
                  controlled through the <strong>Prompt Library</strong>. Go to{" "}
                  <strong>Settings → Prompts</strong> to customize them.
                </p>
              </div>
            </div>
          </div>
        ...
        <ScrollArea className="max-h-[400px]">
          <div className="space-y-4">
            <h3 className="text-sm font-semibold">
              Character Analysis & Suggestions
            </h3>
            {charactersLoading ? (
                  ...
                  characters.map((char: Character) => (
                    ...
                    >
                      ...
                      <div className="text-xs text-white/60">
                        Age: {char.age || "Unknown"} | Personality:{" "}
                        {char.personality || "Unknown"}
                      </div>
                      <div className="text-xs mt-2">
                        <strong>Stats:</strong> Trust{" "}
                        {char.progression?.trust || 0}/100, Affection{" "}
                        {char.progression?.affection || 0}/100
                      </div>
                    </div>
                  ))
                ) : (
        ...
        <div className="space-y-3">
          <div className="rounded-lg border border-blue-500/30 bg-blue-500/10 p-4">
            ...
            <p className="text-xs text-blue-200/80">
              To customize the interview process, edit the{" "}
              <strong>"Character Interview Template"</strong> in the{" "}
              <strong>Prompt Library</strong> (Settings → Prompts).
            </p>
          </div>

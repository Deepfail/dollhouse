...
const NUMERIC_PROGRESSION_KEYS: NumericProgressionKey[] = [
  "affection",
  "trust",
  "intimacy",
  "dominance",
...
const buildSessionOverview = (
  ...
  const latestUserLine = [...recent]
    .reverse()
    .find((msg) => !msg.characterId)?.content;
  const latestCharacterLine = [...recent]
    .reverse()
    .find((msg) => msg.characterId)?.content;
  const userSnippet = latestUserLine
    ? ` You shared: "${truncateText(sanitizeWhitespace(latestUserLine), 120)}".`
    : "";
...
const extractSexualHighlights = (
  ...
  for (const message of messages.slice(-30)) {
    ...
    if (
      normalized &&
      SEXUAL_ACTION_PATTERNS.some((pattern) => pattern.test(normalized))
    ) {
      const speaker =
...
const describeDelta = (
  ...
  if (delta >= 3)
    return `${label} rose to ${Math.round(current)}% (+${delta}).`;
  if (delta <= -3)
    return `${label} dipped to ${Math.round(current)}% (${delta}).`;
  return null;
};
...
const buildCoreStatSummary = (
  ...
  const sorted = NUMERIC_PROGRESSION_KEYS.map((key) => ({
    key,
...
const buildSexStatSummary = (
  ...
  const willingness = describeDelta(
    "Willingness",
    previousStats?.willing,
    currentStats.willing
  );
  if (willingness) parts.push(willingness);
  const stamina = describeDelta(
    "Stamina",
    previousStats?.stamina,
    currentStats.stamina
  );
  if (stamina) parts.push(stamina);
  if (!parts.length && typeof currentStats?.wet === "number") {
    parts.push(
      `${characterName}'s desire is at ${Math.round(currentStats.wet)}%, willingness around ${Math.round(currentStats.willing ?? 0)}%.`
    );
  }
...
const buildFallbackMemoryNote = (
  ...
  const pieces = [sessionOverview, coreSummary, sexSummary]
...
const createStoryEntryForSession = ({
  ...
  const emotionalKeys: NumericProgressionKey[] = [
    "affection",
    "trust",
    "intimacy",
    "dominance",
  ...
  const details = [sessionOverview, coreSummary, sexSummary]
    .map((section) => sanitizeWhitespace(section))
    .filter(Boolean)
    .join("\n\n");
  const emotionsBefore = emotionalKeys.reduce<Record<string, number>>(
    (acc, key) => {
  ...
  const emotionsAfter = emotionalKeys.reduce<Record<string, number>>(
    (acc, key) => {
  ...
  const deltaMagnitude = emotionalKeys.reduce((maxDelta, key) => {
    const before =
      typeof previousProgression?.[key] === "number"
        ? Number(previousProgression?.[key])
        : undefined;
    const after =
      typeof updatedProgression?.[key] === "number"
        ? Number(updatedProgression?.[key])
        : undefined;
    if (before == null || after == null) return maxDelta;
    return Math.max(maxDelta, Math.abs(after - before));
  }, 0);
  const significance: StoryEntry["significance"] =
    deltaMagnitude >= 8
      ? "pivotal"
      : deltaMagnitude >= 5
        ? "high"
...
export function useChat() {
  ...
  const loadSessions = useCallback(
    async (options?: { suppressBroadcast?: boolean; force?: boolean }) => {
      try {
        ...
        for (const sessionRow of sessionRows) {
          ...
          const sceneObjectives = goalRows.reduce(
          ...
          loadedSessions.push({
            ...
            sceneObjectives: Object.keys(sceneObjectives).length
              ? sceneObjectives
              : undefined,
            active: !sessionRow.ended_at, // active if not ended
            messageCount,
            endedAt: sessionRow.ended_at
              ? new Date(parseInt(sessionRow.ended_at))
              : null,
        ...
        if (signature !== lastSignatureRef.current) {
          logger.log(
            ...
            "sessions with participants (changed)"
          );
    ...
    async (sessionId: string): Promise<ChatMessage[]> => {
      try {
        ...
        if (messages.length === 0) {
          logger.log(
            "ℹ️ Session has zero messages (sessionId=",
            sessionId,
            ")"
          );
        }
      ...
      try {
        ...
        if (senderId === "user") {
          try {
            ...
            if (
              lowerContent.includes("dominate") ||
              lowerContent.includes("control")
            ) {
            ...
            try {
              ...
              if (shouldAnalyze) {
                ...
                for (const adjustment of analysis.adjustments) {
                  const target = participantCharacters.find(
                    (c) => c.id === adjustment.characterId
                  );
                  const fullChar = (characters || []).find(
                    (c) => c.id === adjustment.characterId
                  );
                  ...
                  if (adjustment.statAdjustments.stats) {
                    for (const [statKey, delta] of Object.entries(
                      adjustment.statAdjustments.stats
                    )) {
                      if (typeof delta !== "number" || Number.isNaN(delta))
                        continue;
                      if (statKey === "experience") {
                        const nextLevelExp =
                          updatedProgression?.nextLevelExp ?? 100;
                        ...
                        if (updatedProgression) {
                          updatedProgression.level = Math.max(
                        ...
                        const currentValue = Number(
                          updatedStats[typedKey] ?? 0
                        );
                        updatedStats[typedKey] = clampNumber(
                          currentValue + delta
                        );
                      }
                    }
                  ...
                  if (adjustment.statAdjustments.progression) {
                    for (const [progKey, delta] of Object.entries(
                      adjustment.statAdjustments.progression
                    )) {
                      if (typeof delta !== "number" || Number.isNaN(delta))
                        continue;
                      if (isNumericProgressionKey(progKey)) {
                        const current = Number(
                          updatedProgression?.[progKey] ?? 0
                        );
                        updatedProgression[progKey] = clampNumber(
                          current + delta
                        );
                      }
                    }
                  ...
                  const behaviorProfile = createBehaviorProfile(
                    adjustment,
                    fullChar.behaviorProfile
                  );
                  const memories = buildMemoryEntries(
                    fullChar,
                    adjustment.memories
                  );
                ...
                logger.log(
                  "📊 Behavior analysis completed for session",
                  sessionId,
                  "- updated",
                  analysis.adjustments.length,
                  "character profiles"
                );
              }
            } catch (analysisError) {
        ...
        if (senderId === "user" && !options?.copilot) {
          ...
          if (
            ...
            await generateCharacterResponses(
              sessionId,
              session.participantIds,
              content
            );
  ...
  const generateCharacterResponses = useCallback(
    async (sessionId: string, characterIds: string[], userMessage: string) => {
      try {
        ...
        if (sessionChars.length === 0) {
          try {
            ...
            if (st) {
              ...
              const parsed =
                saved && (saved as any).value
                  ? JSON.parse((saved as any).value)
                  : [];
              if (Array.isArray(parsed) && parsed.length) {
                sessionChars = parsed.filter((c: any) =>
        ...
        if (sessionChars.length === 0) {
          try {
            ...
            const legacyChars = await (
              adapterMod as any
            ).StorageAdapter.getCharacters();
            if (Array.isArray(legacyChars) && legacyChars.length) {
              sessionChars = legacyChars.filter((c: any) =>
                characterIds.includes(c.id)
              );
            }
          } catch (e) {
        ...
        if (sessionChars.length === 0) {
          logger.warn(
            "No matching characters found for session participants; skipping AI generation"
          );
          return;
        }
        ...
        try {
          ...
          for (const row of sceneRows) {
            try {
              ...
              if (
                sceneSession.chatSessionId === sessionId &&
                sceneSession.description
              ) {
                ...
                if (sceneSession.hiddenGoals) {
                  ...
                  for (const [charId, goalData] of Object.entries(
                    sceneSession.hiddenGoals as Record<
                      string,
                      { goal: string; priority: string }
                    >
                  )) {
        ...
        try {
          ...
          db.exec({
            ...
            callback: (r: unknown) => {
              sumRows.push(
                r as { summary_text?: string; covered_until?: string }
              );
            },
          });

          sessionSummary = (sumRows[0]?.summary_text || "").trim();
          const lastCovered = sumRows[0]?.covered_until
            ? Number(sumRows[0].covered_until)
            : 0;
          ...
          if (recentMessages.length > SUMMARY_CONFIG.MIN_TOTAL_MESSAGES) {
            const candidateMessages = recentMessages.slice(
              0,
              Math.max(
                0,
                recentMessages.length - SUMMARY_CONFIG.KEEP_RECENT_MESSAGES
              )
            );
            if (candidateMessages.length) {
              const unsummarized = candidateMessages.filter(
                (msg) => timestampMs(msg.timestamp) > lastCovered
              );

              if (unsummarized.length) {
                const textLines = unsummarized
                  .map((m) => {
                    if (
                ...
                const exceedsChunkThreshold =
                  unsummarized.length >= SUMMARY_CONFIG.MIN_CHUNK_MESSAGES ||
                  textBlock.length >= SUMMARY_CONFIG.MIN_CHUNK_CHARS;

                if (exceedsChunkThreshold && textBlock.trim().length) {
                  const summaryPrompt = formatPrompt(
                  ...
                  let newSummary = "";
                  try {
                    newSummary = await AIService.generateResponse(
                      summaryPrompt,
                      undefined,
                      undefined,
                    ...
                    logger.warn(
                      "Summary generation failed; using placeholder.",
                      e
                    );
                    newSummary =
                      "[Summary unavailable; continuing from recent context]";
                  }

                  const lastIncludedTs = unsummarized.reduce(
                    (latest, msg) =>
                      Math.max(latest, timestampMs(msg.timestamp)),
                    lastCovered
                  );
        ...
        for (const character of sessionChars) {
          try {
            ...
            const historyText = conversationHistory
              .map(
                (msg) =>
                  `${nameFor(msg.characterId as string | null)}: ${msg.content}`
              )
              .join("\n");
            ...
            const houseConfig = (await repositoryStorage.get(
              "house_config"
            )) as any;
            ...
            const characterHiddenPrompt =
              character.prompts?.hiddenPrompt?.trim();
...
${character.name}'s response:`;
            ...
            if (response && response.trim()) {
              ...
              const responseTime =
                Date.now() + Math.floor(Math.random() * 3000); // Random delay 0-3s
  ...
  const updateSessionGoal = useCallback(
    async (
      sessionId: string,
      characterId: string,
      goalText: string,
  ...
  const createSession = useCallback(
    async (
      ...
      options?: {
        hiddenGoals?: Record<
          string,
          { goal: string; priority?: "low" | "medium" | "high" }
        >;
      ...
      try {
        ...
        try {
          legacyStorage.setItem("active_chat_session", sessionId);
        } catch {
          /* ignore */
        }
  ...
  const createInterviewSession = useCallback(
    async (characterId: string): Promise<string> => {
      try {
        ...
        const existing = sessions.find(
          (s) =>
            s.type === "interview" &&
            s.participantIds.includes(characterId) &&
            !s.endedAt
        );
        if (existing) {
          logger.log(
            "🔁 Reusing interview session for character",
            characterId,
            existing.id
          );
          setActiveSessionId(existing.id);
        ...
        try {
          ...
          opening = await AIService.generateResponse(
            basePrompt,
            undefined,
            undefined,
          ...
          logger.warn(
            "Failed to generate opening interview question, using fallback",
            e
          );
  ...
  const createGroupSession = useCallback(
    async (
      ...
      const hiddenGoals: Record<
        string,
        { goal: string; priority?: "low" | "medium" | "high" }
      > = {};
      if (roles) {
        for (const [cid, r] of Object.entries(roles)) {
          const text = r.objective
  ...
  const analyzeAndEndSession = useCallback(
    async (sessionId: string): Promise<boolean> => {
      try {
        ...
        const summaryRows: Array<{
          ...
          covered_until?: string | number | null;
        }> = [];
        db.exec({
          sql: "SELECT summary_text, covered_until FROM session_summaries WHERE session_id = ?",
          bind: [sessionId],
        ...
        const endedAtValue = sessionMeta[0]?.ended_at
          ? Number(sessionMeta[0].ended_at)
          : 0;
        const existingSummary = summaryRows[0]?.summary_text?.trim();
        const coveredUntil = summaryRows[0]?.covered_until
          ? Number(summaryRows[0].covered_until)
          : 0;
        ...
        if (
          ...
          logger.log(
            "ℹ️ Session already analyzed; skipping duplicate run for",
            sessionId
          );
        ...
        if (participantCharacters.length && messages.length > 0) {
          ...
          for (const adjustment of analysis.adjustments) {
            ...
            const sexualHighlights = extractSexualHighlights(
              messages,
              fullChar.id
            );
            const coreSummary = buildCoreStatSummary(
              fullChar.name,
              previousProgression,
              updatedProgression
            );
            const sexSummary = buildSexStatSummary(
              fullChar.name,
              previousStats,
              updatedStats,
              sexualHighlights
            ...
            const existingChronicle = Array.isArray(
              updatedProgression?.storyChronicle
            )
            ...
            if (nextChronicle.length > STORY_CHRONICLE_LIMIT) {
              nextChronicle.splice(
                0,
                nextChronicle.length - STORY_CHRONICLE_LIMIT
              );
            }
            ...
            const fallbackMemory = buildFallbackMemoryNote(
              fullChar.name,
              sessionOverview,
              coreSummary,
              sexSummary
            ...
            logger.log(
              `✅ Updated ${fullChar.name} profile from conversation analysis`
            );
          }
        }
  ...
  const ensureIndividualSession = useCallback(
    async (characterId: string): Promise<string> => {

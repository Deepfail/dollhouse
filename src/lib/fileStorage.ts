...
const getElectronAPI = (): ElectronBridge | null => {
...
export class FileStorageSystem {
  ...
  private async ensureDirectoryExists(path: string): Promise<void> {
    const electronAPI = getElectronAPI();
    if (electronAPI) {
      await electronAPI.ensureDirectory(path);
    }
  ...
  async fileExists(filename: keyof StorageFiles | string): Promise<boolean> {
    try {
      ...
      console.error(`Failed to check existence of ${filename}:`, error);
      return false;
    }
  }
  ...
  async createBackup(filename: keyof StorageFiles | string): Promise<void> {
    try {
      ...
      const electronAPI = getElectronAPI();
      if (electronAPI) {
        await electronAPI.writeFile(backupPath, JSON.stringify(data, null, 2));
      } else {
  ...
  async migrateFromLocalStorage(): Promise<void> {
    if (typeof window === 'undefined') {
      console.log('LocalStorage not available in this environment. Skipping migration.');
      return;
    }
    ...
    try {
      const [houseExists, charactersExists] = await Promise.all([
        this.fileExists('house'),
        this.fileExists('characters')
      ]);
      ...
      // Migrate house data
      const houseDataRaw = storage.getItem('character-house');
      if (houseDataRaw) {
        const parsed = JSON.parse(houseDataRaw);

        if (Array.isArray(parsed?.characters)) {
          const { characters: storedCharacters, ...houseInfo } = parsed;
          await this.writeFile('house', houseInfo);
          await this.writeFile('characters', storedCharacters);
          console.log(`Migrated ${storedCharacters.length} characters to separate file`);
        } else {
      ...
      // Migrate remaining keys into settings snapshot
      const allKeys = Object.keys(storage);
      const settings: Record<string, unknown> = {};

      for (const key of allKeys) {
        if (key.startsWith('character-house')) continue;

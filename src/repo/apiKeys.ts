...
export async function listApiKeys() {
  ...
  const rows: any[] = [];
  db.exec({
    sql: 'SELECT id,provider,name,created_at,updated_at FROM api_keys ORDER BY updated_at DESC',
    rowMode: 'object',
...
export async function updateApiKey(id: string, patch: Partial<{ provider: string; name: string; key_encrypted: string }>) {
  ...
  db.exec({
    sql: `UPDATE api_keys SET provider=?, name=?, key_encrypted=?, updated_at=? WHERE id=?`,
    bind: [provider, name, key_encrypted, now, id]
  });
  await saveDatabase();
}
...
export async function getApiKeyById(id: string) {
  ...
  const rows: any[] = [];
  db.exec({
    sql: 'SELECT * FROM api_keys WHERE id=? LIMIT 1',
    bind: [id],
    rowMode: 'object',
    callback: (r: any) => rows.push(r)
  });
...
export async function getApiKeyByProvider(provider: string) {
  ...
  const rows: any[] = [];
  db.exec({
    sql: 'SELECT * FROM api_keys WHERE provider=? ORDER BY updated_at DESC LIMIT 1',
    bind: [provider],
    rowMode: 'object',
    callback: (r: any) => rows.push(r)
  });
